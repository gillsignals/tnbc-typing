...
clust_names <- names(table(tnbc_seurat$seurat_clusters)) 
...
for (i in 1:length(celltype_names)) {
	...
	for (j in 1:length(clust_names)) {
		clust_count <- sum(tnbc_seurat$seurat_clusters == clust_names[j])    # number of cells in given cluster
		overlap_count <- sum(tnbc_seurat$lit_cell_type == celltype_names[i] & tnbc_seurat$seurat_clusters == clust_names[j])    # number of cells of type in cluster
