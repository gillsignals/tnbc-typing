...
# colorblind palette: black, red, skyblue, orange, green, yellow, darkblue, pink 
...
ggsave(celltype_de_heat, filename = "results/type-de-genes-heat-wide.png", width = 10, height = 5)
```
...
ggsave(sc3_de_heat, filename = "results/sc3-de-heat.png", width = 10, height = 7)
```
...
### MIA: Marker-based cell types vs Louvain/Seurat clusters
...
#### Cell overlap: Cell types vs Louvain

```{r}
# initialize output matrix
hyper <- matrix(NA, nrow = length(celltype_names), ncol = length(seurat_names))
...
for (i in 1:length(celltype_names)) {
	...
	for (j in 1:length(seurat_names)) {
		clust_count <- sum(tnbc_seurat$seurat_clusters == seurat_names[j])    # number of cells in given cluster
		overlap_count <- sum(tnbc_seurat$lit_cell_type == celltype_names[i] & tnbc_seurat$seurat_clusters == seurat_names[j])    # number of cells of type in cluster
...
mia_cells_type_louv_pval <- pheatmap(hyper,
		 cluster_cols = FALSE,
		 cluster_rows = FALSE,
		 color = cools_rev,
		 main = "p-values for overlap of cells in types and Louvain clusters")

# inspect heatmap of p-values
mia_cells_type_louv_pval

# save heatmap of p-values
ggsave(mia_cells_type_louv_pval, filename = "results/mia-cells-type-louv-pval.png", height = 4, width = 8)
```

```{r}
# create heatmap of -log10(p-values)
mia_cells_type_louv_log_pval <- pheatmap(-log10(hyper),
		 cluster_cols = FALSE,
		 cluster_rows = FALSE,
		 color = reds,
		 main = "-log10(p-values) for overlap of cells in types and Louvain clusters")

# inspect heatmap of log10 p-values
mia_cells_type_louv_log_pval

# save heatmap of log10 p-values
ggsave(mia_cells_type_louv_log_pval, filename = "results/mia-cells-type-louv-log-pval.png", height = 4, width = 8)
```

```{r}
# create heatmap of sqrt(-log10(p-values))
mia_cells_type_louv_sqrt_log_pval <- pheatmap(sqrt(-log10(hyper)),
		 cluster_cols = FALSE,
		 cluster_rows = FALSE,
		 color = reds,
		 main = "sqrt(-log10(p-values)) for overlap of cells in types and Louvain clusters")

# inspect heatmap of sqrt log10 p-values
mia_cells_type_louv_sqrt_log_pval

# save heatmap of sqrt log10 p-values
ggsave(mia_cells_type_louv_sqrt_log_pval, filename = "results/mia-cells-type-louv-sqrt-log-pval.png", height = 4, width = 8)
```

#### Gene set overlap: Cell types vs Louvain
...
```{r}
# create heatmap of p-values
mia_genes_type_louv_pval <- pheatmap(hyper,
		 cluster_rows = FALSE,
		 cluster_cols = FALSE,
		 color = cools_rev,
		 main = "p-values for overlap of ^ genes in cell types and Louvain clusters")

# inspect heatmap of p-values
mia_genes_type_louv_pval

# save heatmap of p-values
ggsave(mia_genes_type_louv_pval, filename = "results/mia-genes-type-louv-pval.png", width = 8, height = 4)
```

```{r}
# some p values are so small that R treats them as 0, so there's an error with -log10(p)
# assign the 0 values to 1e-200 (less than the smallest reported value of ~1e-143)
hyper <- ifelse(hyper < 1e-200, 1e-200, hyper)

# create heatmap of -log10(p-values)
mia_genes_type_louv_log_pval <- pheatmap(-log10(hyper),
		 cluster_cols = FALSE,
		 cluster_rows = FALSE,
		 color = reds,
		 main = "-log10(p-values) for overlap of ^ genes in cell types and Louvain clusters")

# inspect heatmap of -log10(p-values)
mia_genes_type_louv_log_pval

# save heatmap of -log10(p-values)
ggsave(mia_genes_type_louv_log_pval, filename = "results/mia-genes-type-louv-log-pval.png", width = 8, height = 4)
```

```{r}
# create heatmap of sqrt(-log10(p-values))
mia_genes_type_louv_sqrt_log_pval <- pheatmap(sqrt(-log10(hyper)),
		 cluster_cols = FALSE,
		 cluster_rows = FALSE,
		 color = reds,
		 main = "sqrt(-log10(p-values)) for overlap of ^ genes in types and Louvain clusters")

# inspect heatmap of sqrt(-log10(p-values))
mia_genes_type_louv_sqrt_log_pval

# save heatmap of sqrt(-log10(p-values))
ggsave(mia_genes_type_louv_sqrt_log_pval, filename = "results/mia-genes-type-louv-sqrt-log-pval.png", width = 8, height = 4)
```

### MIA: Marker-based cell types vs k-means/SC3 clusters

```{r}
# create vector of SC3 cluster to iterate over
sc3_names <- names(table(tnbc_seurat$sc3_29_clusters)) 
```


#### Cell overlap: Cell types vs k-means
...
# calculate all hypergeometric p-values for cell overlap between cell type and k-means/SC3 cluster
for (i in 1:length(celltype_names)) {
	celltype_count <- sum(tnbc_seurat$lit_cell_type == celltype_names[i])    # number of cells for given cell type
...
```{r}
# create heatmap of p-values
mia_cells_type_sc3_pval <- pheatmap(hyper,
		 cluster_cols = FALSE,
		 cluster_rows = FALSE,
		 color = cools_rev,
		 main = "p-values for overlap of cells in cell types and k-means/SC3 clusters")

# inspect heatmap of p-values
mia_cells_type_sc3_pval

# save heatmap of sqrt(-log10(p-values))
ggsave(mia_cells_type_sc3_pval, filename = "results/mia-cells-type-sc3-pval.png", width = 8, height = 4)
```

```{r}
# create heatmap of -log10(p-values)
mia_cells_type_sc3_log_pval <- pheatmap(-log10(hyper),
		 cluster_cols = FALSE,
		 cluster_rows = FALSE,
		 color = reds,
		 main = "-log10(p-values) for overlap of cells in cell types and k-means/SC3 clusters")

# inspect heatmap of sqrt(-log10(p-values))
mia_cells_type_sc3_log_pval

# save heatmap of sqrt(-log10(p-values))
ggsave(mia_cells_type_sc3_log_pval, filename = "results/mia-cells-type-sc3-log-pval.png", width = 8, height = 4)
```

```{r}
# create heatmap of sqrt(-log10(p-values))
mia_cells_type_sc3_sqrt_log_pval <- pheatmap(sqrt(-log10(hyper)),
		 cluster_cols = FALSE,
		 cluster_rows = FALSE,
		 color = reds,
		 main = "sqrt(-log10(p-values)) for overlap of cells in types and k-means/SC3 clusters")

# inspect heatmap of sqrt(-log10(p-values))
mia_cells_type_sc3_sqrt_log_pval

# save heatmap of sqrt(-log10(p-values))
ggsave(mia_cells_type_sc3_sqrt_log_pval, filename = "results/mia-cells-type-sc3-sqrt-log-pval.png", width = 8, height = 4)
```

#### Gene set overlap: Cell types vs k-means
...
# calculate all hypergeometric p-values for gene set overlap between cell type and k-means/SC3 cluster
for (i in 1:length(celltype_names)) {
	# list genes upregulated in that cell type
	celltype_genes <- celltype_markers %>%
		filter(cluster == celltype_names[i],
	...
	for (j in 1:length(sc3_names)) {
		...
		sc3_genes <- sc3_k29_markers %>%
			filter(cluster == sc3_names[j],
				   avg_diff > 0) %>%
			pull(gene)
		sc3_count <- length(sc3_genes)    # number of enriched genes in SC3 cluster
		
		# find number of genes enriched in both the SC3 cluster and the cell type
...
```{r}
# create heatmap of p-values
mia_genes_type_sc3_pval <- pheatmap(hyper,
		 cluster_rows = FALSE,
		 cluster_cols = FALSE,
		 color = cools_rev,
...
# inspect heatmap of p-values
mia_genes_type_sc3_pval

# save heatmap of sqrt(-log10(p-values))
ggsave(mia_genes_type_sc3_pval, filename = "results/mia-genes-type-sc3-pval.png", width = 8, height = 4)
```
...
# create heatmap of -log10(p-values)
mia_genes_type_sc3_log_pval <- pheatmap(-log10(hyper),
		 cluster_cols = FALSE,
		 cluster_rows = FALSE,
		 color = reds,
...
# inspect heatmap of -log10(p-values)
mia_genes_type_sc3_log_pval

# save heatmap of -log10(p-values)
ggsave(mia_genes_type_sc3_log_pval, filename = "results/mia-genes-type-sc3-log-pval.png", width = 8, height = 4)
```

```{r}
# create heatmap of sqrt(-log10(p-values))
mia_genes_type_sc3_sqrt_log_pval <- pheatmap(sqrt(-log10(hyper)),
		 cluster_cols = FALSE,
		 cluster_rows = FALSE,
		 color = reds,
...
# inspect heatmap of sqrt(-log10(p-values))
mia_genes_type_sc3_sqrt_log_pval

# save heatmap of sqrt(-log10(p-values))
ggsave(mia_genes_type_sc3_sqrt_log_pval, filename = "results/mia-genes-type-sc3-sqrt-log-pval.png", width = 8, height = 4)
```

### MIA: k-means/SC3 vs Louvain/Seurat clusters

#### Cell overlap: k-means vs Louvain
...
# calculate all hypergeometric p-values for overlap between SC3 and Seurat cluster
...
```{r}
mia_cells_louv_sc3_pval <- pheatmap(hyper,
		 cluster_cols = FALSE,
		 cluster_rows = FALSE,
		 color = cools_rev,
		 main = "p-values for overlap of cells in k-means/SC3 and Louvain clusters")

# inspect heatmap of sqrt(-log10(p-values))
mia_cells_louv_sc3_pval

# save heatmap of sqrt(-log10(p-values))
ggsave(mia_cells_louv_sc3_pval, filename = "results/mia-cells-louv-sc3-pval.png", width = 8, height = 4)
```

```{r}
# create heatmap of -log10(p-values)
mia_cells_louv_sc3_log_pval <- pheatmap(-log10(hyper),
		 cluster_cols = FALSE,
		 cluster_rows = FALSE,
		 color = reds,
		 main = "-log10(p-values) for overlap of cells in k-means/SC3 and Louvain clusters")

# inspect heatmap of -log10(p-values)
mia_cells_louv_sc3_log_pval

# save heatmap of -log10(p-values)
ggsave(mia_cells_louv_sc3_log_pval, filename = "results/mia-cells-louv-sc3-log-pval.png", width = 8, height = 4)
```

```{r}
# create heatmap of sqrt(-log10(p-values))
mia_cells_louv_sc3_sqrt_log_pval <- pheatmap(sqrt(-log10(hyper)),
		 cluster_cols = FALSE,
		 cluster_rows = FALSE,
		 color = reds,
		 main = "sqrt(-log10(p-values)) for overlap of cells in k-means/SC3 and Louvain clusters")

# inspect heatmap of sqrt(-log10(p-values))
mia_cells_louv_sc3_sqrt_log_pval

# save heatmap of sqrt(-log10(p-values))
ggsave(mia_cells_louv_sc3_sqrt_log_pval, filename = "results/mia-cells-louv-sc3-sqrt-log-pval.png", width = 8, height = 4)
```

#### Gene set overlap: k-means vs Louvain
...
for (i in 1:length(sc3_names)) {
	# list genes upregulated in that SC3 cluster
	sc3_genes <- sc3_k29_markers %>%
		filter(cluster == sc3_names[i],
			   avg_diff > 0) %>%
		pull(gene)
	celltype_count <- length(sc3_genes)    # number of enriched genes in SC3 cluster
	...
	for (j in 1:length(seurat_names)) {
		...
		# find number of genes enriched in both the Seurat cluster and the SC3 cluster
...
```{r}
# create heatmap of p-values
mia_genes_louv_sc3_pval <- pheatmap(hyper,
		 cluster_rows = FALSE,
		 cluster_cols = FALSE,
		 color = cools_rev,
		 main = "p-values for overlap of ^ genes in cell types and Louvain clusters")
...
# inspect heatmap of p-values)
mia_genes_louv_sc3_pval

# save heatmap of p-values
ggsave(mia_genes_louv_sc3_pval, filename = "results/mia-genes-louv-sc3-pval.png", width = 8, height = 4)
```
...
# create heatmap of -log10(p-values)
mia_genes_louv_sc3_log_pval <- pheatmap(-log10(hyper),
		 cluster_cols = FALSE,
		 cluster_rows = FALSE,
		 color = reds,
		 main = "-log10(p-values) for overlap of ^ genes in cell types and Louvain clusters")
...
# inspect heatmap of -log10(p-values)
mia_genes_louv_sc3_log_pval

# save heatmap of -log10(p-values)
ggsave(mia_genes_louv_sc3_log_pval, filename = "results/mia-genes-louv-sc3-log-pval.png", width = 8, height = 4)
```

```{r}
# create heatmap of sqrt(-log10(p-values))
mia_genes_louv_sc3_sqrt_log_pval <- pheatmap(sqrt(-log10(hyper)),
		 cluster_cols = FALSE,
		 cluster_rows = FALSE,
		 color = reds,
		 main = "sqrt(-log10(p-values)) for overlap of ^ genes in types and Louvain clusters")
...
# inspect heatmap of sqrt(-log10(p-values))
mia_genes_louv_sc3_sqrt_log_pval

# save heatmap of sqrt(-log10(p-values))
ggsave(mia_genes_louv_sc3_sqrt_log_pval, filename = "results/mia-genes-louv-sc3-sqrt-log-pval.png", width = 8, height = 4)
```

Some SC3 clusters do not have a significant overlap of differentially expressed genes with any Louvain clusters (for example, SC3-2/5/16/17/22). One of these clusters (SC3-2) only had one cell, so differentially expressed genes could not be calculated. Others had a small number of cells - the largest SC3 cluster that had little overlap with Louvain clusters (SC3-17) had 20 cells. This lack of significant overlap in differentially expressed genes likely reflects lack of power due to the small size of these clusters, and again suggests that the SC3 estimated optimal value of k split cells into too many clusters.
